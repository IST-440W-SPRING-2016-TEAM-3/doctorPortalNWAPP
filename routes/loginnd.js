...
router.post('/', function(req, res, next) {
	...
	if (userData.email && userData.password) {
		login.checkUser(userData.email, userData.password, function(err, user) {
			...
			if (user) {
                console.log(user);
				if (user.useraccess !== "dn") {
					res.send("Username or Password is incorrect. Please try again. Click<a href=\"http://127.0.0.1:8000\"> here</a> to try logging in again.");
				} else {
                    req.session.useraccess = user.useraccess;
